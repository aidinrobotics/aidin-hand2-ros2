...
`aidin_hand2_description`은 로봇 핸드의 URDF·xacro와 mesh를 제공합니다.
...
|---|---|
| [aidin_hand2_description/urdf/aidin_hand2.urdf.xacro](urdf/aidin_hand2.urdf.xacro) | 기본 launch가 사용하는 최상위 URDF |
...
| [aidin_hand2_description/meshes/](meshes) | visual·collision mesh |
...
`aidin_hand2_description`은 로봇 핸드의 모델과 제어 설정을 xacro 파일로 제공합니다.
...
왼손이면 첫 번째와 세 번째 파일을 불러옵니다. 아래의 `$(find aidin_hand2_description)`은 설치된
package의 공유 디렉터리를 찾는 xacro 표현식입니다. 사용자의 workspace 절대 경로를 적지 않아도 됩니다.

다음 내용을 기존 URDF의 `<robot>` 요소 안에 추가합니다. 아래의 바깥 `<robot>`은 위치를 보여 주기
위한 것으로, 기존 `<robot>` 안에 다시 중첩하지 않습니다. 기존 요소에 `xmlns:xacro`가 없다면 추가하십시오.
예제의 `my_arm_tool0`는 기존 로봇의 링크 이름으로 바꿉니다.
...
맞게 바꾸십시오. 두 매크로의 `prefix`는 왼손이면 `left_`, 오른손이면 `right_`로 일치시킵니다.

오른손은 `aidin_hand2_right.urdf.xacro`를 불러와 `aidin_hand2_right`를 호출하고,
`prefix="right_"`, `name="right_hand_control"`, `hand_side="right"`로 지정합니다.
양손이면 좌우 모델 파일을 모두 불러오고, 모델·제어 설정 호출을 각각 한 쌍씩 추가합니다.
제어 설정 파일은 한 번만 include하면 됩니다.
...
`hand_side`는 `left`·`right` 중 하나여야 하고 `can_interface`는 비워 둘 수 없습니다.
`auto_home`의 시작 조건은 [3. home](../aidin_hand2_hardware/README.ko.md#3-home)에 있습니다.
