...
// Chain interface
//   claims and exports <side>_<active_joint>/position x16 on the JointPositionController
// Command input
      ...
      [this](const std::shared_ptr<sensor_msgs::msg::JointState> message) {
        ...
        if (!resolve_joint_state_command(
          ...
          RCLCPP_WARN_THROTTLE(
            ...
            "JointPosition upper command dropped — name and position differ in length, or a name repeats");
          return;
        }
...
protected:
    ...
    const rclcpp::Time &, const rclcpp::Duration &) override
  {
    ...
    if (!has_complete_target) {
      RCLCPP_WARN_THROTTLE(
        get_node()->get_logger(), *get_node()->get_clock(), 5000,
        "JointPosition upper target dropped — not all 16 values are finite");
      std::fill(
