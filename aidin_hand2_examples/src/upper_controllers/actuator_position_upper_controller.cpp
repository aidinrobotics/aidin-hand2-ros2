...
// Chain interface
//   claims and exports <side>_<actuator>/position_cnt x16 on the ActuatorPositionController
// Command input
      ...
      [this](const std::shared_ptr<sensor_msgs::msg::JointState> message) {
        ...
        if (!resolve_joint_state_command(
          ...
          RCLCPP_WARN_THROTTLE(
            ...
            "ActuatorPosition upper command dropped — name and position differ in length, or a name repeats");
          return;
        }
...
protected:
    ...
    const rclcpp::Time &, const rclcpp::Duration &) override
  {
    ...
    // ------------------------------------ WRITE -----------------------------------
    // Write the algorithm here, the input is in reference_interfaces_[0..15] (target_position_cnt)
    ...
    if (!has_complete_target) {
      RCLCPP_WARN_THROTTLE(
        get_node()->get_logger(), *get_node()->get_clock(), 5000,
        "ActuatorPosition upper target dropped — not all 16 values are finite");
      std::fill(
