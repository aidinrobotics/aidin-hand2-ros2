...
public:
    ...
    const rclcpp_lifecycle::State &) override
  {
    ...
    for (const char * actuator : kActuatorBaseNames) {
      command_names_.push_back(side + "_" + actuator);
      reference_suffixes_.push_back(side + "_" + actuator + "/effort_pct");
    }
    ...
    command_subscriber_ = get_node()->create_subscription<sensor_msgs::msg::JointState>(
      ...
      [this](const std::shared_ptr<sensor_msgs::msg::JointState> message) {
        ...
        if (!resolve_joint_state_command(
              *message, command_names_, JointStateField::kEffort, command.values)) {
          RCLCPP_WARN_THROTTLE(
            get_node()->get_logger(), *get_node()->get_clock(), 5000,
            "ActuatorEffort upper command dropped — name and effort differ in length, or a name repeats");
          return;
        }
...
protected:
    ...
    const rclcpp::Time &, const rclcpp::Duration &) override
  {
    ...
    // ------------------------------------ WRITE -----------------------------------
    // Write the algorithm here, the input is in reference_interfaces_[0..15] (target_effort_pct)
    ...
    if (!has_complete_target) {
      RCLCPP_WARN_THROTTLE(
        get_node()->get_logger(), *get_node()->get_clock(), 5000,
        "ActuatorEffort upper target dropped — not all 16 values are finite");
      std::fill(
...
private:
  ...
  realtime_tools::RealtimeBuffer<JointStateCommand<ah2::kActuatorCount>> command_buffer_;
...
PLUGINLIB_EXPORT_CLASS(
  aidin_hand2_examples::ActuatorEffortUpperController,
